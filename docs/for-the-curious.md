...
| **Every assistant wants the same setup written differently.** The tool speaks one standard (MCP), but Claude Code, Copilot in VS Code, Cursor and Claude Desktop each keep that setting in a different file, under a different name, with paths you have to get exactly right. | Press **Connect** in the header: pick your assistant, and the tool writes the setting for you — adding itself without disturbing any other assistant already set up there. It also shows a dot when an assistant is working on the same data as you. |
